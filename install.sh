...
usage() {
    echo -e "${CYAN}Uso:${NC} $0 [--fresh|--upgrade]"
    echo ""
    echo -e "  ${VERDE}--fresh${NC}   Instalación limpia (respaldar y reemplazar configs)"
    echo -e "  ${VERDE}--upgrade${NC} Actualización (solo añade lo nuevo, preserva cambios)"
    echo -e "  ${VERDE}sin flag${NC}   Pregunta interactivamente"
    exit 0
}
...
case "${1:-}" in
    ...
    -h|--help) usage ;;
    "") ;;
    *) error "Flag desconocido: $1" && usage ;;
esac

if [[ -z "$MODE" ]]; then
    echo -e "${MAGENTA}¿Cómo querés instalar?${NC}"
    echo -e "  ${VERDE}1)${NC} Fresh  — Instalación limpia (respaldar y reemplazar todo)"
    echo -e "  ${VERDE}2)${NC} Upgrade — Solo añadir modificaciones nuevas"
    echo ""
    read -rp $'Opción [1/2]: ' CHOICE
    case "$CHOICE" in
        1|fresh|f) MODE="fresh" ;;
        2|upgrade|u) MODE="upgrade" ;;
        *) error "Opción inválida" && exit 1 ;;
    esac
fi

info "Modo: ${MAGENTA}$MODE${NC}"
...
if [[ $EUID -eq 0 ]]; then
    error "NO ejecutes como root."
    exit 1
fi

if ! command -v pacman &>/dev/null; then
    error "Solo funciona en Arch Linux / derivados."
    exit 1
fi
...
if [[ "$MODE" == "fresh" ]]; then
    ...
    info "Instalando paquetes oficiales faltantes..."
    MISSING=()
    for pkg in "${PACMAN_PKGS[@]}"; do
        pacman -Qi "$pkg" &>/dev/null || MISSING+=("$pkg")
    done
    if [[ ${#MISSING[@]} -gt 0 ]]; then
        sudo pacman -S --needed --noconfirm --ask 4 "${MISSING[@]}"
...
CONFIG_DIRS=(hypr waybar rofi swaync fastfetch qt6ct Kvantum gtk-3.0 gtk-4.0 \
             hypr-wallpicker ghostty btop wlogout hyprwat quickshell)
...
if [[ "$MODE" == "fresh" ]]; then
    ...
    info "Modo upgrade: solo archivos nuevos, se preservan tus cambios"
    ...
    copy_config_if_new() {
        ...
        if [[ ! -e "$dst" ]]; then
            ...
            local changed
            changed=$(diff -rq "$src" "$dst" 2>/dev/null || true)
            if [[ -n "$changed" ]]; then
                warn "↻ .config/$1 tiene cambios locales — se preserva"
            fi
        fi
    }
    copy_home_if_new() {
    ...
    for dir in "${CONFIG_DIRS[@]}"; do
        copy_config_if_new "$dir"
    done

    if [[ -d "$DOTFILES_DIR/.config/quickshell" ]] && [[ ! -d "$HOME/.config/quickshell" ]]; then
        mkdir -p "$HOME/.config/quickshell"
        rsync -a --exclude='.git' "$DOTFILES_DIR/.config/quickshell/" "$HOME/.config/quickshell/"
        ok "+ .config/quickshell/ (nuevo)"
    fi

    copy_home_if_new ".local/share/color-schemes"
    if [[ ! -f "$HOME/.config/kdeglobals" ]]; then
        cp "$DOTFILES_DIR/.config/kdeglobals" "$HOME/.config/kdeglobals"
        ok "+ .config/kdeglobals (nuevo)"
    fi
fi
...
if [[ ! -f "$HOME/.config/custom_wall_paper/wallpicker" ]]; then
    ...
    (cd /tmp/hypr-wallpicker && make -j"$(nproc)") || warn "Falló compilación (puedes compilarlo manualmente)"
...
if [[ "$MODE" == "fresh" ]] || [[ "$MODE" == "upgrade" ]]; then
    ...
    if [[ ${#MISSING_AUR[@]} -gt 0 ]]; then
        info "Instalando ${#MISSING_AUR[@]} paquetes AUR..."
        set +e
        $AUR_HELPER -S --needed --noconfirm "${MISSING_AUR[@]}"
        set -e
        ok "Paquetes AUR procesados"
    else
        ok "Todos los paquetes AUR ya están instalados"
    fi
fi
...
if [[ "$GPU" = "nvidia" ]]; then
    if ! grep -q "LIBVA_DRIVER_NAME" "$HOME/.config/hypr/environments.conf" 2>/dev/null; then
        cat >> "$HOME/.config/hypr/environments.conf" <<-EOF
...
EOF
        ...
        ok "Variables NVIDIA ya configuradas"
    fi
else
...
if [[ "$MODE" == "fresh" ]]; then
    ...
    for svc in bluetooth NetworkManager sddm; do
        sudo systemctl is-enabled "$svc" &>/dev/null || warn "$svc no está habilitado (corré 'sudo systemctl enable --now $svc')"
    done
    systemctl --user is-enabled pipewire &>/dev/null || warn "pipewire no habilitado (corré 'systemctl --user enable --now pipewire pipewire-pulse wireplumber')"
fi
...
echo ""
echo -e "${AMARILLO}Modo:${NC} ${MAGENTA}$MODE${NC}"
echo ""
if [[ "$MODE" == "fresh" ]]; then
